...
<main>
    <div class="container">
        <h1 class="fs40 mb32"><?php echo $data['text']['recovery'] ?></h1>
        <form  class="form-1" method="POST" action="<?php Navigate::view('public/reset_password') ?>">
            <input class=" mb8" required class="mb8" type="email" name="email" placeholder="<?php echo $data['text']['enter_email'] ?>" />
            <button class="btn-1 fs24" name="submit"><?php echo $data['text']['submit'] ?></button>
        </form>
<?php
if (isset($_POST["submit"])) {
      try {
        ...
        if (!isset($user['name'])) {
            ?>
            <p class="failed_message bold"><?php echo $data['text']['user_not_found'] ?> </p>
          <?php
        }else {
